...
void Solution([[maybe_unused]] std::istream& cin, [[maybe_unused]] std::ostream& cout) {
  int n, m; cin >> n >> m;
  std::vector<std::pair<int, ii>> edges(m);
  FOR(i, 0, m-1) {
    int v, u, w; cin >> v >> u >> w;
    edges[i] = {w, ii{v, u}};
  }
  ...
  FOR(i0, 0, m-n+1) {
    std::fill(RNG(dsu.r_), 1);
    std::iota(RNG(dsu.p_), 0);
    ...
    bool ok = true;
    FOR(v, 2, n) ok &= (dsu.Get(1) == dsu.Get(v));
    if (ok) min_diff = std::min(min_diff, CL(mx) - CL(mn));
  }
  if (min_diff == LLONG_MAX) cout << "NO"sv << '\n';
  else cout << "YES" << '\n' << min_diff << '\n';
}
