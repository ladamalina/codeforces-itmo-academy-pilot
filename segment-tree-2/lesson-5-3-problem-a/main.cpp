...
template<typename T>
class SegTree { // Присваивание и максимальный отрезок
public:
  struct Node {
    int cnt = 0;
    T sum=0, p_sum=0, s_sum=0, max_sum=0;
    T set = NO_OP;
  };
  ...
  T Get(int l, int r) {
    return Get(l, r, 0, 0, size_).max_sum;
  }
...
private:
  ...
  static Node SetOp(const Node& x, T v) {
    if (v == NO_OP) return x;
    return {
      x.cnt,
      x.cnt * v,
  ...
  static Node Combine(const Node& x, const Node& l, const Node& r) {
    Node m{
      l.cnt + r.cnt,
      l.sum + r.sum,
      std::max(l.p_sum, l.sum+r.p_sum),
      std::max(r.s_sum, l.s_sum+r.sum),
      std::max(l.s_sum+r.p_sum, std::max(l.max_sum, r.max_sum)),
      x.set
    };
    return SetOp(m, x.set);
  }
  ...
  void Propagate(int x, int lx, int rx) {
    ...
    t_[2*x+1] = SetOp(t_[2*x+1], t_[x].set);
    t_[2*x+2] = SetOp(t_[2*x+2], t_[x].set);
