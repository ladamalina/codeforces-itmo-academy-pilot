...
template<typename T>
class SegTree { // Присваивание и минимум
public:
  struct Node {
    T min = INF;
    T set = NO_OP;
...
private:
  ...
  static Node Combine(const Node& x, const Node& l, const Node& r) {
    return {
      (x.set == NO_OP ? std::min(l.min, r.min) : x.set),
      x.set
    };
  }
  ...
  void Init(const std::vector<T>& a, int x, int lx, int rx) {
    if (lx + 1 == rx) {
      if (lx < CI(a.size())) t_[x] = {a[lx], NO_OP};
    } else {
  ...
  void Propagate(int x, int lx, int rx) {
    ...
    t_[2*x+1] = {t_[x].set, t_[x].set};
    t_[2*x+2] = {t_[x].set, t_[x].set};
    t_[x].set = NO_OP;
  }
