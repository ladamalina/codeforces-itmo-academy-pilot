...
public:
  ...
  void Modify(int l, int r) {
    Modify(l, r, 1, 0, size_);
  }

  int Get(int i) {
    return Get(i+1, 1, 0, size_);
  }
...
private:
  ...
  void Propagate(int x, int lx, int rx) {
    ...
    t_[(x<<1)] = {t_[(x<<1)].cnt, t_[(x<<1)].cnt - t_[(x<<1)].sum, !t_[(x<<1)].rev};
    t_[(x<<1)|1] = {t_[(x<<1)|1].cnt, t_[(x<<1)|1].cnt - t_[(x<<1)|1].sum, !t_[(x<<1)|1].rev};
    t_[x] = {t_[x].cnt, t_[(x<<1)].sum + t_[(x<<1)|1].sum, !t_[x].rev};
  }
  ...
  int Get(int i, int x, int lx, int rx) {
    ...
    return Get(i-t_[(x<<1)].sum, (x<<1)|1, m, rx);
  }
};
