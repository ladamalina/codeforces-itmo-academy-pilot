...
public:
  explicit SegTree(const std::vector<T>& a) : NO_OP(std::numeric_limits<T>::min()) {
    while (size_ < SZ(a)) size_ <<= 1;
    t_.resize(size_ << 1, NO_OP);
    Init(a, 1, 0, size_);
  }
...
private:
  ...
  T Get(int i, int x, int lx, int rx) {
    ...
    const auto m = (lx + rx) >> 1;
    if (i < m) {
      return (t_[x] == NO_OP) ? Get(i, (x<<1), lx, m) : t_[x];
    }
    return (t_[x] == NO_OP) ? Get(i, (x<<1)|1, m, rx) : t_[x];
  }
};
