...
template<typename T>
class SegTree { // Присваивание и сумма
public:
  struct Node {
    int cnt = 0;
    T sum = 0;
...
private:
  ...
  static Node Combine(const Node& x, const Node& l, const Node& r) {
    return {
      l.cnt+r.cnt,
      (x.set==NO_OP ? (l.sum+r.sum) : x.set*(l.cnt+r.cnt)),
      x.set
    };
  }
  ...
  void Init(const std::vector<T>& a, int x, int lx, int rx) {
    if (lx + 1 == rx) {
      if (lx < CI(a.size())) t_[x] = {1, a[lx], NO_OP};
    } else {
  ...
  void Propagate(int x, int lx, int rx) {
    ...
    t_[2*x+1].sum = t_[2*x+1].cnt * t_[x].set;
    t_[2*x+1].set = t_[x].set;
    
    t_[2*x+2].sum = t_[2*x+2].cnt * t_[x].set;
    t_[2*x+2].set = t_[x].set;
