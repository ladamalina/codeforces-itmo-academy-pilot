...
constexpr ll MOD = CL(1e9)+7;

template<typename T>
class SegTree { // Умножение и сумма
public:
  struct Node {
    T sum = 0;
    T mult = 1;
  ...
  T GetSum(int l, int r) {
    return GetSum(l, r, 0, 0, size_).sum;
  }
...
private:
  ...
  void Modify(int l, int r, T v, int x, int lx, int rx) {
    if (rx <= l || lx >= r) return;
    if (rx <= r && lx >= l) {
      t_[x].sum = (t_[x].sum * v)%MOD;
      t_[x].mult = (t_[x].mult * v)%MOD;
      return;
    }
    const auto m = (lx + rx) / 2;
  ...
  Node GetSum(int l, int r, int x, int lx, int rx) {
    if (rx <= l || lx >= r) return {};
    if (rx <= r && lx >= l) {
      return t_[x];
    }
    const auto m = (lx + rx) / 2;
    const auto m1 = GetSum(l, r, 2*x+1, lx, m);
    const auto m2 = GetSum(l, r, 2*x+2, m, rx);
