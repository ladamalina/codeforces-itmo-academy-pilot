...
private:
  ...
  void Propagate(int x, int lx, int rx) {
    ...
    if (t_[x].set == 0) {
      t_[(x<<1)] = {t_[(x<<1)].len, 0, 0, false, false, t_[x].set};
      t_[(x<<1)|1] = {t_[(x<<1)|1].len, 0, 0, false, false, t_[x].set};
      t_[x] = {t_[x].len, 0, 0, false, false, NO_OP};
    }
    if (t_[x].set == 1) {
      t_[(x<<1)] = {t_[(x<<1)].len, t_[(x<<1)].len, 1, true, true, t_[x].set};
      t_[(x<<1)|1] = {t_[(x<<1)|1].len, t_[(x<<1)|1].len, 1, true, true, t_[x].set};
