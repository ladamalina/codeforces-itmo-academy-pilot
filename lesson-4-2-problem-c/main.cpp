...
class SegmentTree {
 public:
  ...
  [[nodiscard]] int GetLowerBound(int v) {
    return GetLowerBound(v, 0, 0, static_cast<int>(size_));
  }
 ...
 private:
  ...
  [[nodiscard]] int GetLowerBound(int v, int x, int lx, int rx) {
    if (lx + 1 == rx) {
      return tree_[x].max >= v ? lx : -1;
    }
    const auto m = (lx + rx) / 2;
    if (tree_[2*x+1].max >= v) {
      return GetLowerBound(v, 2*x+1, lx, m);
    } else if (tree_[2*x+2].max >= v) {
      return GetLowerBound(v, 2*x+2, m, rx);
    }
    return -1;
  }
