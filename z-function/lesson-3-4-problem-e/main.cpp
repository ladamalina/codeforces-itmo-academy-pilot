...
void Solution([[maybe_unused]] std::istream& cin, [[maybe_unused]] std::ostream& cout) {
  ...
  const auto n = SZ(s), m = n * 2 + 1;
  
  const std::string tr(t.rbegin(), t.rend());
  const auto tr_s = tr + std::string(1, '_') + s;
  const auto t_s = t + std::string(1, '_') + s;
  const auto z_tr_s = ZFunction(tr_s), z_t_s = ZFunction(t_s);

  auto res_k = (s == tr) ? n : -1;
  FOR(k, 1, n-1) {
    if (z_tr_s[n+1] >= k && z_t_s[m-(n-k)] == n-k) {
      res_k = k;
      break;
    }
  }
