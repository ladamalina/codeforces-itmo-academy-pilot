...
void Solution([[maybe_unused]] std::istream& cin, [[maybe_unused]] std::ostream& cout) {
  ...
  while (n_tests--) {
    ...
    const auto st = s + t, ts = t + s;
    const auto zst = ZFunction(st), zts = ZFunction(ts);

    // std::cerr << "st=" << st << '\n';
    // for (const auto x : zst) std::cerr << x << ' ';
    // std::cerr << '\n';
    // std::cerr << "ts=" << ts << '\n';
    // for (const auto x : zts) std::cerr << x << ' ';
    // std::cerr << '\n';
