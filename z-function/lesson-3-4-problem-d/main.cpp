...
void Solution([[maybe_unused]] std::istream& cin, [[maybe_unused]] std::ostream& cout) {
  ...
  s += std::string(1, '_') + s_rev;
  const auto z = ZFunction(s);

  int max_len = 1;
  FOR(i, 1, n-1) {
    const auto half_len = i, tail_len = n - i;
    if (half_len <= tail_len) { // even
      if (z[(n*2+1)-half_len*2] >= half_len) {
        max_len = std::max(max_len, half_len*2);
      }
    }
    if (half_len <= tail_len - 1) { // odd
      if (z[(n*2+1)-(half_len*2+1)] >= half_len) {
        max_len = std::max(max_len, half_len*2+1);
      }
    }
  }
  cout << max_len << '\n';
