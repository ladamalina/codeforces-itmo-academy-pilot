...
void Solution([[maybe_unused]] std::istream& cin, [[maybe_unused]] std::ostream& cout) {
  ...
  while (n_tests--) {
    ...
    const auto sn = SZ(s), tn = SZ(t), m = sn + tn + 1;
    const auto s_t = s + std::string(1, '_') + t;
    const auto t_s = t + std::string(1, '_') + s;
    const auto z_s_t = ZFunction(s_t), z_t_s = ZFunction(t_s);
    
    { // s includes t
      const auto found = std::find(z_t_s.begin() + tn + 1, z_t_s.end(), tn) != z_t_s.end();
      if (found) {
        cout << s << '\n';
        continue;
      }
    ...
    { // t includes s
      const auto found = std::find(z_s_t.begin() + sn + 1, z_s_t.end(), sn) != z_s_t.end();
      if (found) {
        cout << t << '\n';
        continue;
      }
    ...
    if (st_len != -1 && ts_len != -1) {
      if (st_len >= ts_len)
        cout << s << t.substr(st_len) << '\n';
      else
        cout << t << s.substr(ts_len) << '\n';
    } else if (st_len != -1) {
      cout << s << t.substr(st_len) << '\n';
    } else if (ts_len != -1) {
      cout << t << s.substr(ts_len) << '\n';
