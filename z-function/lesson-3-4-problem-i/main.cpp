...
void Solution([[maybe_unused]] std::istream& cin, [[maybe_unused]] std::ostream& cout) {
  std::string t, p; cin >> t >> p;
  int k; cin >> k;
  const auto tn = SZ(t), pn = SZ(p);
  ...
  const auto p_t = p + "_"s + t;
  const auto pr_tr = std::string(p.rbegin(), p.rend()) + "_"s + std::string(t.rbegin(), t.rend());
  const auto z_p_t = ZFunction(p_t), z_pr_tr = ZFunction(pr_tr);
  const auto m = SZ(p_t);

  vi res;
  FOR(ti, 0, tn-pn) {
    const auto z1 = z_p_t[pn + 1 + ti];
    const auto ti_end = ti + pn - 1;
    const auto tj = m - ti_end - 1;
    const auto z2 = z_pr_tr[tj];
    if (z1 + z2 + k >= pn)
      res.push_back(ti + 1);
  }
