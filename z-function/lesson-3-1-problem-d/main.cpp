...
bool Match(const std::string& s, const std::string& t, int i) {
  auto si = i, ti = 0;
  while (si < SZ(s) && ti < SZ(t)) {
    if (s[si++] != t[ti++])
      return false;
  }
  return ti == SZ(t);
...
void Solution([[maybe_unused]] std::istream& cin, [[maybe_unused]] std::ostream& cout) {
  ...
  FOR(ti, 1, tn) {
    ...
    int res = 0;
    FOR(i, 0, n-1) {
      const auto it = std::lower_bound(RNG(pos), i);
      const auto j = (it == pos.end()) ? n-1 : *it + SZ(t) - 2;
      res += j - i + 1;
    }
    cout << res << '\n';
  }
